...
export async function updateUserSettings(formData: FormData) {
  ...
  try {
    ...
    if (existingUser.length > 0) {
      // Account exists, do nothing and allow login
      return { success: true, message: "Account already exists. Logged in." }
    }

    // Update user information
    let updateQuery = `
      UPDATE users 
      SET username = $1, email = $2, whatsapp = $3, updated_at = CURRENT_TIMESTAMP
    `
    const params = [username, email, whatsapp]
...
export async function getUserSettings() {
  ...
  try {
    const result = await query("SELECT username, email, whatsapp FROM users WHERE id = $1", [user.id])
